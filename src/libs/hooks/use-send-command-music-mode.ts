...
export function useSendCommandMusicMode() {
	...
	const [rawSendCommand, { loading, error, data }] = usePostData<
		(LampStateResponse | LampErrorResponse)[]
	>(`${LAMP_API}/lamp/music-mode`);
	...
	useEffectUpdate(
		([oldLoading]) => {
			...
			const newStates = data
				.map(lampStateResponse => {
					if (isLampErrorResponse(lampStateResponse))
						return console.error('Error on lamp method:', lampStateResponse);
