...
const SelectTemperature: SelectTemperatureComponent = ({ defaultValue, onChange = () => {} }) => {
	function handleColorTemperatureChange(x: number) {
		...
		const roundedTemperature = Math.max(
			MIN_TEMPERATURE,
			Math.min(MAX_TEMPEARTURE, Math.round(temperature)),
		);
		onChange(roundedTemperature);
	}

	const getTempeartureFunc = React.useMemo(
		() => (x: number) => {
	...
	return (
		<StripColor
			...
			defaultValue={
				((defaultValue || MIN_TEMPERATURE) - MIN_TEMPERATURE) / (MAX_TEMPEARTURE - MIN_TEMPERATURE)
			}
		/>
	);
};
