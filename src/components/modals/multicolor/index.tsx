	...
	function validateData() {
		const errors = framesData.map((frameData, index) => {
			...
			if (frameData.duration === undefined)
				return baseErrorMessage + 'você deve fornecer uma duração';
			if (frameData.brightness === undefined)
				return baseErrorMessage + 'você deve fornecer uma luminosidade';
			if (frameData.mode === undefined)
				return baseErrorMessage + 'você deve especificar um modo cor';
			if (frameData.mode !== 7 && !frameData.value === undefined)
				return baseErrorMessage + 'você deve especificar uma cor';
			if (frameData.duration < 50)
				return baseErrorMessage + 'a duração deve ser maior que 50 milissegundos';
			return null;
		});
